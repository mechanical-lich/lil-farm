...
## Achievements
Thirty-nine of them, on the journal's fourth tab.
...
- **Counting hangs off three seams, not thirty.**  The farmer's task pipeline is where every job's effect lands and where everything that reaches the bag passes through exactly once — including what the farmhands gather, which arrives via a `gather` or an `unload`.  The shop is the second, for animals and hires.  The day streak is the third.
